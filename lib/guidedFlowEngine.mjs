...
export function emptyFlowState(flow) {
  return {
...
export function runGuidedFlow({
  ...
  const baseState = startNew
    ...
    : {
        ...
        current_node: stored.current_node || dbState?.current_node || flow.entry,
  ...
  function handoff(reason, nodeId, extra = {}) {
    const state = {
      ...baseState,
      current_node: nodeId || baseState.current_node || flow.entry,
      mode: "handoff",
      step: reason,
      last_action: reason,
      resolved: false,
  ...
  function renderNode(nodeId, patch = {}) {
    ...
    if (node.type === "human") {
      return handoff("human_requested", nodeId, { labels: ["human_requested"], state: patch });
    }
  ...
  const currentNodeId = baseState.current_node || flow.entry;
  const currentNode = flow.nodes[currentNodeId] || flow.nodes[flow.entry];
  ...
  if (startNew || !dbState?.active_flow_id || baseState.resolved || greetingOnly) {
    return renderNode(flow.entry, { path: [], form_data: {}, selected_option: null, llm_turns: 0 });
  }
